...
=======
  ...
  const FETCH_TIMEOUT_MS = 12000; // 12s timeout on all Paystack API calls
  ...
  /** Fetch with a hard timeout to avoid hanging under load */
  ...
  // In-memory payment session: one active payment per streamId
  ...
  // Tracks (ip → { count, windowStart }) to prevent payment flooding
  const rateLimitMap = new Map<string, { count: number; windowStart: number }>();
  const RATE_LIMIT_WINDOW = 60_000;   // 1 minute window
  const RATE_LIMIT_MAX    = 20;       // 20 init requests / IP / minute
  ...
  // Purge stale rate-limit and session entries every 5 minutes to prevent memory leaks
  setInterval(() => {
  ...
  // POST /api/paystack/init — create a Paystack transaction, return reference + QR url
  app.post("/api/paystack/init", requireAuth, async (req: Request, res: Response): Promise<void> => {
    ...
    // Paystack uses kobo/pesewas (smallest unit) — multiply by 100
    const amountSmallest = Math.round(amount * 100);
    // Currency: use what's configured for this Paystack account (default NGN — most widely supported)
    ...
    try {
      ...
      if (!data.status || !data.data) {
        // Surface the actual Paystack error message to help debugging
        res.status(502).json({
      ...
      // QR encodes our scan-tracking redirect URL
  ...
  // GET /api/paystack/scan — viewer scanned the QR, log it, redirect to Paystack
  ...
  // GET /api/paystack/paid — Paystack callback after payment
    ...
    try {
      ...
      if (data.status && data.data?.status === "success") {
        ...
        if (sess) {
          sess.status = "paid";
          sess.payerName = name;
          sess.payerEmail = customer?.email || null;
        }
  ...
  // POST /api/paystack/webhook — Paystack server-to-server webhook
  ...
  // DELETE /api/paystack/reset — streamer resets the payment session
...
>>>>>>> 19b122fd6ce18926233c5ba49e1d5a5f66c8b4a6
